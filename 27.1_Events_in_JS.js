...
2. timestamp
 ...
 4. target
   - Description: The `target` property refers to the DOM element that triggered the event. This can be 
     useful when multiple elements might trigger the same event listener.
 ...
 5. toElement
   ...
   - The `toElement` property is used in mouse events and refers to the element that the pointer has 
     moved to during a `mouseover` or `mouseout` event.
   - Usage:
 ...
 6. srcElement
   ...
   - `srcElement` is an alias for `target` in some browsers (especially older versions of Internet 
      Explorer). It returns the element that triggered the event.
 ...
 7. currentTarget
   ...
   - The `currentTarget` property refers to the element to which the event handler is attached, as 
     opposed to `target`, which refers to the element that triggered the event.
 ...
 8. clientX
   ...
   - The `clientX` property indicates the horizontal coordinate of the mouse pointer, relative to the 
     viewport (excluding scroll offsets).
...
- The `clientY` property indicates the vertical coordinate of the mouse pointer, relative to the viewport.
- Usage:
 ...
 10. screenX
   - The `screenX` property provides the X-coordinate of the mouse pointer relative to the entire screen 
   (not just the browser's viewport).
 ...
 11. screenY
   - The `screenY` property provides the Y-coordinate of the mouse pointer relative to the 
     screen.
 ...
 12. altKey
   ...
   - The `altKey` property is a boolean that indicates whether the "Alt" key was pressed when the event 
     occurred.
   - Usage:
 ...
 13. ctrlKey
    ...
    - The `ctrlKey` property is a boolean that indicates whether the "Control" key was pressed during 
      the event.
    - Usage:
 ...
 14. shiftKey
   ...
   - The `shiftKey` property is a boolean that indicates whether the "Shift" key was pressed during the 
     event.
 ...
 15. keyCode
   ...
   - The `keyCode` property indicates the numeric code for the key that was pressed during keyboard 
     events like `keydown` or `keyup`. It is deprecated and replaced by `event.code` or `event.key` in 
     modern browsers.
 ...
 Additional Notes:
- `clientX`, `clientY`, `screenX`, and `screenY` are properties related to mouse events and provide 
   coordinates for the pointer position.

- `altKey`, `ctrlKey`, and `shiftKey` are modifier keys often used in combination with other actions, 
   such as clicking or typing.

- `keyCode` is not recommended for new code due to its deprecation in modern browsers.

These event properties provide valuable information for creating interactive and responsive user 
interfaces.
